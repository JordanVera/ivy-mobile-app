        ...
        {communityActivity.map((a) => (
          <View
            ...
            <View className="h-10 w-10 items-center justify-center rounded-full bg-amber-100 dark:bg-amber-900/30">
              <IconSymbol name="person.crop.circle.fill" size={28} color="#b45309" />
            </View>
            <View className="flex-1">
              <View className="flex-row items-center justify-between">
                <IvyText className="font-semibold text-zinc-900 dark:text-white">{a.name}</IvyText>
                <IvyText className="text-xs text-zinc-500 dark:text-zinc-400">{a.time}</IvyText>
              </View>
              <IvyText className="mt-1 text-sm text-zinc-600 dark:text-zinc-300">{a.snippet}</IvyText>
            </View>
          </View>
        ))}
        </ScrollView>
